...
function ReadContract({ getReadContractInfo, readContract, loading, contractIsVerified }: ContractProps) {
  ...
  console.log('asdfasdfasdfasdf', `${BackendURL}/address/readContract/${tokenAddress}`)
  ...
  React.useEffect(() => {
    getReadContractInfo(tokenAddress)
        ...
        for (let i = 0; i < readContract.length; i++) {
          ...
          if ('method_result' in method) {
            ...
            var queryFormExample = {
              ...
              apiUrl: `${BackendURL}/address/readContract/${tokenAddress}`,
              panelTitle: method['method_name'],
            }
            accordion.children = (
              <ContractQueryForm
  ...
  }, [tokenAddress, loading, getReadContractInfo])
  return !loading ? (
    contractIsVerified ? (
      <Box>
        <Box>
